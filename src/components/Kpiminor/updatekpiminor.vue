        ...
        <v-col cols="12" sm="6" md="4">
          <v-menu :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
            <template v-slot:activator="{ on, attrs }">
              <v-text-field v-bind:value="getConvertDate(kpiminor.datecurrent)" label="วันที่บันทึกรูปภาพ" prepend-icon="event" readonly v-bind="attrs" v-on="on" disabled></v-text-field>
            </template>
...
export default {
  ...
  props: ["tab_index", "kpiminor"],
  computed: {
    ...mapGetters({
      resultdialog: "ui/resultdialog",
      listimagekpiminor: "kpiminor/listimgekpiminors"
    })
  },
  watch: {
    date(val) {
      console.log(val);
      this.kpiminor.datecurrent = val;
    },
    //ดูการเปลี่ยนค่าแล้ว
    resultdialog() {
    ...
    listimagekpiminor() {
      this.length = this.listimagekpiminor.length;
    },
    onboarding(val) {
      //รับค่ามาค้นหาใน arrayimage เพื่อที่จะเอาbase64 ไปใช้ต่อ
      var newarray = this.listimagekpiminor.filter(el => {
        return el.index == val;
      });
      // console.log("arraytest  " + newarray[0].id);
      this.image = "";
      this.image = `${this.urldefualt}api/imageskpiminor/`.concat(
        this.listimagekpiminor[val].pathimg
      );
    }
  },
  mounted() {
    if (this.listimagekpiminor.length != 0) {
      this.image = `${this.urldefualt}api/imageskpiminor/`.concat(
        this.listimagekpiminor[0].pathimg
      );
      this.length = this.listimagekpiminor.length;
    }
  },
  methods: {
    ...
    async MachineupdateHandlerSubmit() {
      ...
      for (let file of this.files) {
        ...
        formDatapic.append("pathname", "Kpiminor/" + file.name);
        formDatapic.append("update_by", userId);
        formDatapic.append("uuid", this.kpiminor.id);
      }
      ...
      this.$store
        ...
        .then(res => {
          ...
          this.$store.dispatch("kpiminor/getimages", this.kpiminor.id);
          this.$store.dispatch(
    ...
    confirmdelete(value) {
      if (value.resultdialogs == true) {
        ...
        this.$store
          .dispatch("kpiminor/deletekpiminorimages", newarray[0].id)
          .then(res => {
            this.$store.dispatch("kpiminor/getimages", this.kpiminor.id);
            this.$store.dispatch(
              "snackbar/setSnackbar", {
