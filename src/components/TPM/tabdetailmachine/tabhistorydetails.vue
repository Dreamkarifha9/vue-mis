...
export default {
  ...
  mounted() {
    if (this.listhistoryimg.length > 0) {
      this.image2 = `${this.urldefualt}api/imagesplan/`.concat(
        this.listhistoryimg[0].pathimage
      );
    }
    this.lengthimg2 = this.listhistoryimg.length;
  },
  computed: {
    ...mapGetters({
      resultdialog: "ui/resultdialog",
      listhistoryimg: "tpm/listimageshistorys"
    })
  },
  watch: {
    listhistoryimg() {
      var listimg2 = this.listhistoryimg;
      this.lengthimg2 = listimg2.length;
    },
    onboarding2(val) {
      if (val < this.listhistoryimg.length) {
        this.image2 = "";
        this.image2 = `${this.urldefualt}api/imageshistory/`.concat(
          this.listhistoryimg[val].pathimage
        );
      }
    }
  },
