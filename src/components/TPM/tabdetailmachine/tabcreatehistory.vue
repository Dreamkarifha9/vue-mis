...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      listimagehistory: "tpm/listimageshistorys",
  ...
  watch: {
    ...
    listimagehistory() {
      this.length = this.listimagehistory.length;
    },
    onboarding(val) {
      //รับค่ามาค้นหาใน arrayimage เพื่อที่จะเอาbase64 ไปใช้ต่อ
      var newarray = this.listimagehistory.filter(el => {
        return el.index == val;
      });
      // console.log("arraytest  " + newarray[0].id);

      this.image = "";
      this.image = `${this.urldefualt}api/imageshistory/`.concat(
        this.listimagehistory[val].pathimage
      );
    }
  },
  mounted() {
    if (this.listimagehistory.length != 0) {
      this.image = `${this.urldefualt}api/imageshistory/`.concat(
        this.listimagehistory[0].pathimage
      );
    }
    this.length = this.listimagehistory.length;
  },
  ...
  methods: {
    ...
    async MachineuploadHandlerSubmit() {
      ...
      for (let file of this.files) {
        ...
        formDatapic.append("pathname", "uploadhistory/" + file.name);
        formDatapic.append("create_by", userId);
      }
      this.$store
        .dispatch("tpm/createimghistory", formDatapic)
        .then(res => {
          ...
          this.$store.dispatch("tpm/getimageshistory", this.machine.idmachine);
    ...
    confirmdelete(value) {
      if (this.activetab == "planimage") {
        if (value.resultdialogs == true) {
          ...
          this.$store
            .dispatch("tpm/deleteimageshistory", newarray[0].id)
            .then(res => {
              this.$store.dispatch(
                "tpm/getimageshistory",
